App.controller('livetrackingCtrl', function ($rootScope, $scope, $http,$timeout, $cookies, $cookieStore, MY_CONSTANT, $state, ngDialog,$filter, socketFactory, $window, $interval,$sce) {
	...
	$scope.getPaymentMode =  function(){
		...
		.then(function successCallback(data, status) {
			$rootScope.$apply(function () { 
					...
					//console.log(data.payment_type[0].card_used);
	...
	$scope.getDataAndshowPopup = function(){
		...
		// var ride_detail = {
		// 	name : $scope.booking.user_name ,
		// 	mobile : $scope.booking.user_mobile ,
		// 	pickup : $scope.booking.pickup_text,
		// 	drop : $scope.booking.drop_text,
		// 	cartype : $scope.booking.car_type
		// }
		// 	localStorage.setItem('ride_detail', JSON.stringify(ride_detail));
			// var ride_detail = JSON.parse(localStorage.getItem('ride_detail'));
	...
	$scope.BookRideRightNow = function(){
		...
		if(!$scope.booking.cardId){
			...
			alert('Please Add a new card.');
			// console.log($scope.booking.card_user_type);	
			$('#loading').modal('hide');
	...
	$scope.requestRideAPI = function(){
		...
		if(!$scope.booking.cardId){
			...
			// console.log($scope.booking.cardId);
		...
		if(when=='now'){
			...
			if($scope.settings.bothEnabled == true ){
				if(!$scope.booking.returnRide){
					// console.log($scope.Cards.length,'11')
						$scope.showPaymentStep();
						console.log($scope.settings.CorpCardEnabled,'111')
			...
			if(!$scope.validateBookingData()){
				...
				if($scope.settings.bothEnabled == true){
					...
					$scope.booking.card_user_type = ($scope.settings.CorpCardEnabled == true) ? 1 : 0;
					// console.log('Trying to book a ride later with card type as',$scope.booking.card_user_type );
					$('#loading').modal('show');	
	...
	$scope.BookRideLater = function(){
		...
		// console.log('Corporate has ',$scope.Cards.length,' Cards | And Rider has',$scope.RiderCards.length,' Cards')
		/*if($scope.RiderCards.length == 0){
			$scope.showRiderCardError();
		}*/
		...
		if(!$scope.booking.cardId && !$scope.booking.returnRide){
			$('#request_right_now').modal('hide');
			console.log(!$scope.booking.cardId && !$scope.booking.returnRide,'bb')
			console.log($scope.booking.cardId ,$scope.booking.returnRide,'sss')
			alert('Please select a valid card.');
			$('#loading').modal('hide');
			return false;
		}
	...
	$scope.scheduleRequestAPI = function() {
		...
		if(!$scope.booking.cardId && !$scope.booking.returnRide){
			console.log(!$scope.booking.cardId && !$scope.booking.returnRide,'aa')
			$('#request_right_now').modal('hide');
			$scope.resetForBookingOnError();
			alert('Please select a valid card.');
			$('#loading').modal('hide');
	...
	//$scope.addMarker(haightAshbury);
			...
			setTimeout(function(){
				if($scope.tripDetailMode){
					if($scope.hasDriver == 'YES'){
						//console.log('Show Driver for this Ride - ',daa.session_id,', and clean other markers');
						
						if($scope.driverEnable){
							// console.log('Trigger all');
						}else{
							// console.log('Trigger '+daa.session_id);
						...
						if(!$scope.driverEnable){
							// console.log('Hide Driver for this Ride - ',daa.session_id,', and clean driver markers');
		...
		$scope.expandDisc = function (daa) {
				...
				if($scope.ride_old){
					if($scope.ride_old.session_id == $scope.ride_new.session_id){
						if($scope.ride_old.status != $scope.ride_new.status){
							//console.log('Ride Status has been changed to ',$scope.ride_new.status);
							if($scope.ride_new.status == 4){
								var rts;
								if($scope.ride_new.scheduled == 1){
									//console.log('Show latest Scheduled Ride');
									...
									//console.log('Show latest Regular Ride');
								...
								//console.log('New Ride to show ',rts);
								setTimeout(function(){
									$scope.expandDisc(rts);
									setTimeout(function(){
										$scope.expandDisc(rts);
			...
			//	console.log('REGULAR START TIME ',daa.start_time);
			...
			//	console.log('REGULAR RAW TIME ',$scope.pickup_time_raw);
			}else{
			//	console.log('SCHEDULED RIDE TIME ',daa.pickup_time);
			...
			//	console.log('SCHEDULED RIDE TIME ',$scope.pickup_time_raw);
			...
			if($scope.tripDetailMode){
				if($scope.hasDriver == 'YES'){
					if($scope.driverEnable){
						...
						// console.log('Show '+daa.session_id);
						$scope.search_session_id = daa.session_id;
					}
					//console.log('Toggle Status',$scope.driverEnable);
					socketFactory.emit('corporateDriverAvailablity', {session_id:$scope.search_session_id,limit:$scope.driverLimit,offset:$scope.driverSkip, searchFlag:$scope.dsearchFlag, searchString:$scope.dsearchString});
					
				}else{
					if(!$scope.driverEnable){
						//console.log('Expaneded Hide Driver for this Ride - ',daa.session_id,', and clean driver markers');
						$scope.disableDrivers();
						$scope.cleanDrivers();
					}
				}
		...
		socketFactory.on('corporateScheduledRides', function(data) {
				...
				ScheduledRides.forEach(function(ride) {
					...
					if(ride.session_id == 10007 ){
						// console.log(ride.session_id,ride.pickup_time);
						}
		...
		$scope.monitorLiveData = function (){
			...
			$scope.enableDrivers = function(){
				...
				//console.log('Checking Driver Status -',$scope.driverEnable);
				...
				if($scope.driverEnable == 1){
					...
					//console.log('Destroying',gettingDrivers);
					gettingDrivers = 0;
					$scope.cleanDrivers();
				}
			...
			$scope.disableDrivers = function(){
				//console.log('Stop fetching Drivers');
				$interval.cancel(gettingDrivers);		
			...
			$scope.cleanDrivers = function(id){
				if(id){
					// console.log('Cleaning except ',id);
				}else{
				...
				//console.log('Cleaning markers -',markers.length);
			...
			socketFactory.on('corporateDriverAvailablity', function(data) {
				...
				//console.log('Received Data ',data[0].data);
				...
				if($scope.availableTo >= $scope.totalAvailableCars){
					...
					$scope.hideNextAV = 1;
					//console.log('Max reached ',$scope.availableFrom,' - ',$scope.availableTo);
				}
				...
				//console.log('now page |',$scope.availableCurrentPage,'|');
						...
						map.setZoom(16);
							...
							*/
							/** get all drivers coordinates and set map bounds **/
								...
								// console.log('Results ',pointers);
								$scope.setBoundsNow(pointers);
							
						}else{
							// console.log('Same results for ',$scope.dsearchString,$scope.avail.length);
						}
					}
				}
		...
		directionsService.route(request, function (response, status) {
			...
			if (status === google.maps.DirectionsStatus.OK) {
				...
				if($scope.fakeThisId == $scope.routeState.id){
					//console.log('Yes Need to include Driver for Session -',$scope.fakeThisId);
	...
	$scope.setBoundsNow = function(pointers) {
		...
		if(pointers.length > 0){
			...
			map.setCenter(bounds.getCenter());
			// console.log('Fitting new bounds',bounds);
			map.fitBounds(bounds);
			zoomlevel = map.getZoom();
			//map.setZoom(zoomlevel);
			$scope.driverZoom = zoomlevel;
			$scope.lastBounds = {searched:$scope.dsearchString,size:pointers.length,zoom:zoomlevel}
			// console.log('Map Centered Now for searched Drivers ',zoomlevel,$scope.lastBounds);
		}else{
			// console.log('Driver not found',pointers);
		}
	...
	$scope.validateRebookingDateTime = function(){
		...
		// console.log(datedata[0],$scope.rebooking);
