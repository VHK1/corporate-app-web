App.controller('livetrackingCtrl', function ($rootScope, $scope, $http, $cookies, $cookieStore, MY_CONSTANT, $state, ngDialog,$filter, socketFactory, $window, $interval,$sce) {
	...
	$scope.carsOptions = [
		{'car_type':1,'car_name':'QS','car_select':'QS Select','car_text':'max 4 seats'},
		{'car_type':2,'car_name':'QLE','car_select':'QLE Select','car_text':'max 4 seats'},		
		{'car_type':3,'car_name':'LUXE','car_select':'LUXE Select','car_text':'max 4 seats'},
		{'car_type':4,'car_name':'GRANDE','car_select':'GRANDE Select','car_text':'max 4 seats'},
		{'car_type':5,'car_name':'WAV','car_select':'WAV Select','car_text':'max 4 seats'},
		{'car_type':7,'car_name':'QXL','car_select':'QXL Select','car_text':'max 4 seats'},
		{'car_type':6,'car_name':'ELITE','car_select':'ELITE Select','car_text':'max 4 seats'},
		
	];
	$scope.selectCarType = function() {
		console.log('Changed Car TYPE');
		$scope.booking.select_car_type = $scope.booking.car_type;
		
	
		$scope.booking.car_type = $scope.booking.car_type;
		console.log(' Fare -' , $scope.booking.car_type ,$scope.carsOptions[$scope.booking.car_type-1].car_name, $scope.carsOptions[$scope.booking.car_type-1].value_regular,$scope.carsOptions[$scope.booking.car_type-1].value_scheduled);
	...
	/*if(localStorage.getItem('justLogin') == 1){
		console.log('logged In showing New ride form');
	...
	$scope.reTryRide = function(CurrentRide){
		console.log('return requested for ',CurrentRide);
	...
	$scope.swapLocationforReturn = function(){
		...
		console.log('RETURN -',drop_address,pickup_address);
	...
	$scope.swapLocation = function(){
		...
		//$scope.booking.pickup_text =  drop_text;
		//$scope.booking.drop_text  = pickup_text;
		console.log(drop_address,pickup_address);
	...
	$scope.checkName = function(){
		...
		if($scope.booking.user_mobile.length >9){
			//console.log('checking -',$scope.booking.user_mobile,$scope.booking.user_name);
			if($scope.Users){
				$scope.Users.forEach(function(user) {
					if(user.user_mobile.indexOf($scope.booking.user_mobile)>=0){
						//console.log(user.user_mobile.indexOf($scope.booking.user_mobile),user.user_mobile,user.user_name);
	...
	$scope.selectCarClass = function(type,fare,sche_fare){
		//console.log('Selecting Car Class ',type,fare,sche_fare);
		$scope.booking.car_type = type;
		//console.log('Later Fare -',$scope.later_car_options[type-1].car_name,$scope.later_car_options[type-1].value);
	...
	$scope.loadPickup = function(){
		...
		$scope.booking.offset = new Date().getTimezoneOffset();
			...
			$.each(autocompletesWraps, function(index, name) {
				...
				google.maps.event.addListener(autocomplete[name], 'place_changed', function() {
					...
					if(name == 'pickup' || name == 'return-pickup'){
						...
						//console.log(' Passed address - ',place.formatted_address);
	...
	$scope.createUser = function() {
		...
		if($scope.booking.proceed ==  1){
			...
			console.log('No we still need to add it.');
		}
						...
						user_mobile :  $scope.promoCountryCode +$scope.booking.user_mobile,
								...
								if (data.flag == 101) {
									...
									if($scope.booking.autoHitApi){
										...
										$scope.booking.driver_id = '';
										$scope.booking.select_car_type = $scope.booking.car_type;
										//console.log('car n fare',$scope.booking.car_type,fare,sche_fare);
										...
										//console.log('car n fare',$scope.booking.car_type,fare,sche_fare);
										//console.log('Checking if Car Selected',$scope.booking.carSelected,$scope.booking.car_type);
										$scope.getDriversData();
										/*if($scope.booking.carSelected == 0){
											if($scope.booking.car_type){
												$scope.getDriversData();
											}
										}*/
	...
	$scope.getDriversData = function(){
			...
			if($scope.booking.user_id){
				...
				console.log( $scope.booking.user_id, 'user_id not found');
			}
	}
	...
	$scope.BookRideRightNow = function(){
		...
		//console.log($scope.booking.card_user_type,$scope.booking.cardId);
	...
	$scope.requestRideAPI = function(){
		...
		//console.log('Final Booking Info ',$scope.booking);
		
		
		if($scope.notFav){
			//console.log('Car selected instead of Driver',$scope.booking.driver_id,$scope.booking.is_fav);
			var params = {
				web_access_token:  $cookieStore.get('web_access_token'),				
			...
			//console.log('Driver selected ',$scope.booking.driver_id,$scope.booking.is_fav);
			var params = {
				web_access_token:  $cookieStore.get('web_access_token'),				
				current_latitude:$scope.booking.current_latitude,
				current_longitude:$scope.booking.current_longitude,
			...
			$scope.buttonNowClicked = 1;
					$.post(MY_CONSTANT.urlC+ 'request_ride', params) .then(function successCallback(data) {
							...
							if (data.flag == 101) {
								...
								$scope.resetForBookingOnError();
								$('#loading').modal('hide');	
								alert('Your sub merchant account not added yet.');
								//$scope.showPaymentAlert();
								$scope.$apply();
								 return;
								//alert(data.error || data.message);
	...
	$scope.timeSelected = function(when){
		...
		console.log('Card Mode ',$scope.booking.cardSelected,$scope.booking.card_user_type);
		...
		if(when=='now'){
			...
			if(!$scope.booking.returnRide){
				...
				/*if($scope.RiderCards.length == 0 && $scope.Cards.length == 0){
					console.log('NO CARD Added YET');				
					$scope.showCardAlert();						
					$('#loading').modal('hide');				
				}else{
					$scope.showPaymentStep();
			...
			if (data.flag == 807) {
				...
				corporateCards.forEach(function(cardInfo) {
					...
					if(Date.parse(d.card_added_on)){
						...
						console.log(d.card_added_on,dt,dt.getTimezoneOffset(),d.offset);
				...
				riderCards.forEach(function(card) {
					...
					if(Date.parse(r.card_added_on) && (r.offset)){
						...
						console.log(r.card_added_on,dt,dt.getTimezoneOffset(),r.offset);
	...
	$scope.slect = function(driver,notfav){
		...
		if(driver == ''){
			console.log('EMPTY CAR TYPE or  DRIVER');
			return false;
		}
	...
	//	console.log(driver,notfav, $scope.booking);
		if(!$scope.booking.autoHitApi){
			$scope.slectNext();
		}
	}
	...
	$scope.validateDateTime = function(){
		...
		if(datedata[0] === dateToday){
		  ...
		  var m =d.getMinutes();
			...
			//console.log('current',h+':'+m);
			//console.log('should be ',sh+':'+sm);
			//console.log('given ',$scope.booking.time_hour+':'+$scope.booking.time_minutes);
			...
			if(hour_difference == 0){
				...
				if(min_difference<=0){
					//console.log('Hour is same, and min difference ',60 - min_difference);
				}else {
					//console.log('ERROR : Hour is same, and min difference ',60 - min_difference);
					return 0;
				}
				...
				//console.log('Time selected is in future');
			}
			
		}else{
			//console.log('Date Different',datedata[0], dateToday);
	...
	$scope.sendRiderCardAuthorization = function(when){
		...
		$('#request_ride_later').modal('hide');
		$('#payment_step').modal('hide');
		console.log('Authorizing for ',when);
	...
	$scope.scheduleRequestAPI = function() {
		...
		console.log('Booking Info ',$scope.booking);
		console.log($scope.booking.card_user_type,$scope.booking.cardId);
		if($scope.notFav){
			var params = {
				web_access_token:  $cookieStore.get('web_access_token'),				
	...
	$scope.reAddUser = function() {
		...
		//console.log('User '+$scope.booking.user_id+' Adding to Account');
		$scope.otpMode = 1;
		$scope.userToAdd = $scope.booking.user_id;
		$scope.otpToAdd = '';
	...
	$scope.findPlaceNow = function(type){
			...
			if(type == "pickup"){
				address = $('#pickup').val();
				//console.log($('#pickup').val(),'| typed -'+$scope.booking.pickup_text);
				if((!address) || (address =='')){
				...
				address = $('#drop').val();
				//console.log($('#drop').val(),'| typed -'+$scope.booking.drop_text);
				if((!address) || (address =='')){
	...
	//$scope.addMarker(haightAshbury);
		...
		$scope.expandDisc = function (daa) {
			...
			if($scope.tripDetailMode){
				...
				console.log(daa,daa.estimate_pickup_time);
				$scope.classExpand = 1;
			}
			...
			//console.log( $scope.driver_image );
			...
			if(daa.is_schedule == '0'){
				...
				$scope.pickup_time_raw =  daa.pickup_time;
				
				/*	var dt = new Date(daa.pickup_time)
				dt.setMinutes(dt.getMinutes() + dt.getTimezoneOffset())
				var raw = dt.toISOString();
				$scope.pickup_time_raw =  raw;
				$scope.pickup_time_raw =  raw;*/
			}
			...
			$scope.schedule_calender_accept_timezone = ' ';
			if(daa.accept_time!=null){
				$scope.schedule_calender_accept_time = daa.accept_time;
				$scope.schedule_calender_accept_timezone = new Date($scope.schedule_calender_accept_time).toLocaleString('en', {timeZoneName:'short'}).split(' ').pop();
				if(($scope.schedule_calender_accept_timezone == "India Standard Time") || ($scope.schedule_calender_accept_timezone == "GMT+5:30")){
					$scope.schedule_calender_accept_timezone = 'IST';
				}	
			}
            ...
            if (daa.pickup_id) {
                ...
                for (let ride_history of completedrides_history) {
                    if (daa.driver_id == ride_history.driver_id) {
						...
						//console.log('Fare',total,ride_history.fare_calculated,ride_history);
		...
		$scope.pageChanged = function(currentPage,type) {
			...
			if(type == "scheduled"){
				...
				/*
				if($scope.scheduledFrom >= $scope.totalItemsScheduled){
					console.log('BEFORE MAX RECORD ',$scope.scheduledFrom);
					$scope.scheduledFrom = $scope.totalItemsScheduled;
					
					console.log('AT MAX RECORD ',$scope.scheduledFrom);
				}else{
				...
				console.log('SCHEDULE SHOWING : '+ $scope.scheduledFrom + ' - ' + $scope.scheduledTo);
				*/
				
				console.log('Page changed to: ' + $scope.scheduledCurrentPage,$scope.totalItemsScheduled / 15 + 1);
				...
				for (var i = 1; i <= $scope.totalItemsScheduled / 15; i++) {
					if ($scope.scheduledCurrentPage == i) {
						...
						console.log('scheduleSkip Offset changed to: ' + $scope.scheduleSkip + ' of ',$scope.totalItemsScheduled,' Set Page - '+i);
					}
				}
				...
				socketFactory.emit('corporateScheduledRides', {corporate_id:$scope.driverDetails.corporate_id, limit:15,offset:$scope.scheduleSkip, sort_by:'' , sort_order:'' , searchFlag:$scope.searchFlag, searchString:$scope.searchString, requestType:5});
					...
					console.log($scope.regularFrom,$scope.regularTo, $scope.totalItemsRegular); 
					*/
					
					
					console.log('Page changed to: ' + $scope.regularCurrentPage);
					$scope.hideNext = 0;
					...
					for (var i = 1; i <= $scope.totalItemsRegular / 15 + 1; i++) {
						if ($scope.regularCurrentPage == i) {
							...
							console.log('Regular Offset changed to: ' + $scope.regularSkip+' - '+i);
							//$scope.$apply();
					...
					console.log($scope.completedFrom,$scope.completedTo, $scope.totalItemsCompleted); 
					console.log('Page changed to: ' + $scope.completedCurrentPage);
					...
					for (var i = 1; i <= $scope.totalItemsCompleted / 15 + 1; i++) {
						if ($scope.completedCurrentPage == i) {
							...
							console.log('completed Offset changed to: ' + $scope.completedSkip+' - '+i);
							//$scope.$apply();
					...
					$scope.ongoingCurrentPage = currentPage;
					console.log('Page changed to: ' + $scope.ongoingCurrentPage);
					$scope.hideNextongoing = 0;
					
					if(parseInt($scope.totalItemsOngoing / 15 + 1) <= currentPage){
						$scope.hideNextongoing = 1;
					}
					for (var i = 1; i <= $scope.totalItemsOngoing / 15 + 1; i++) {
						if ($scope.ongoingCurrentPage == i) {
							$scope.ongoingSkip = 15 * (i - 1);
							console.log('ongoing Offset changed to: ' + $scope.ongoingSkip+' - '+i);
							//$scope.$apply();
							
						}
					}
				//socketFactory.emit('corporateOngoingRides', { corporate_id:$scope.driverDetails.corporate_id, limit:15,offset:$scope.ongoingSkip, sort_by:'' , sort_order:'' , searchFlag:$scope.searchFlag, searchString:$scope.searchString, requestType:1});
					...
					console.log('Page changed to: ' + $scope.availableCurrentPage);
					$scope.hideNextAV = 0;
					...
					if(!$scope.dsearchFlag){
						//$scope.totalAvailableCars = $scope.totalAvailableCars/ 200 + 1;
					}
					...
					for (var i = 1; i <= $scope.totalAvailableCars; i++) {
						if ($scope.availableCurrentPage == i) {
							if(!$scope.dsearchFlag){	
								//$scope.driverSkip = 200 * (i-1);
								//console.log('Cars Offset changed to: ' + $scope.driverSkip+' - '+i);
							}
						}
					}
					...
					if($scope.dsearchFlag){		
						console.log($scope.driverToFocus);
						
						//$scope.driverToFocus[$scope.availableCurrentPage-1];
						
						console.log($scope.availableCurrentPage-1, $scope.driverToFocus[$scope.availableCurrentPage-1], $scope.driverToFocus[$scope.availableCurrentPage-1].current_location_latitude,$scope.driverToFocus[$scope.availableCurrentPage-1].current_location_longitude);					
						
						var newlatlng = new google.maps.LatLng($scope.driverToFocus[$scope.availableCurrentPage-1].current_location_latitude, $scope.driverToFocus[$scope.availableCurrentPage-1].current_location_longitude);
			
						map.setCenter(newlatlng);
						$scope.driverZoom = map.getZoom();
						map.setZoom($scope.driverZoom);
					}else{
						//console.log("NOT Searching Drivers");
						//socketFactory.emit('corporateDriverAvailablity', {limit:200,offset:$scope.driverSkip, searchFlag:$scope.dsearchFlag, searchString:$scope.dsearchString});
		...
		socketFactory.on('corporateRequestedDriverList', function(data) {
			...
			socketFactory.on('corporateOngoingRides', function(data) {
                   
					
					
					
					var OngoingData = data[0].data.paginated_rides;
					
					$scope.onGoingRidePag = [];
					OngoingData.forEach(function(ride) {
						
						if(ride.ride_status==0 || ride.ride_status==4){
							
                        }else{
							var r = ride;
							
							$scope.onGoingRidePag.push(r);
						}
					});	
					
									
					
					$scope.totalItemsOngoing = $scope.onGoingRidePag.length;
					
					$scope.ongoingPages = parseInt($scope.totalItemsOngoing/15 + 1);
					
					
					if($scope.ongoingCurrentPage == 1){
						$scope.hidePrevongoing = 1;
						$scope.hideNextongoing = 0;
					}else if($scope.ongoingCurrentPage == $scope.ongoingPages){
						$scope.hidePrevongoing = 0;
						$scope.hideNextongoing = 1;
					}else{
						$scope.hidePrevongoing = 0;
						$scope.hideNextongoing = 0;
					...
					if($scope.ongoingPages <=1){
						$scope.hidePrevongoing = 1;
						$scope.hideNextongoing = 1;
					}
					
					//console.log('----------Ongoing',$scope.totalItemsOngoing);
					...
					$scope.busy = $scope.onGoingRidePag;
                    var ids = [];
                    ...
                    if($scope.select.selectedIndex == 1 || $scope.select.selectedIndex == 3){ // for busy and all
                        ...
                        for (var i = 0; i < $scope.busy.length; i++) {
                            ...
                            var infowindow = new google.maps.InfoWindow({
                                ...
                                "<span></span>  <span>" + $scope.busy[i].driver_name + "</span><br>" +
                                "<span></span>  <span>" + $scope.busy[i].referral_code + "</span><br>" +
		...
		socketFactory.on('corporateCompletedRequests', function(data) {
			...
			$scope.totalItemsCompleted = data[0].data.count;
				...
				CompletedData.forEach(function(ride) {
					...
					if(ride.pickup_time){
						...
						r.pickup_time =  ride.pickup_time;
					...
					if(i == 1){
						/*if($scope.classExpand != 1){
							console.log('No Ride in focus ,',$scope.classExpand);
						//	$scope.expandDisc(r);
						}*/
			...
			//console.log($scope.completedCurrentPage);
			if($scope.completedCurrentPage == 1){
				$scope.hidePrevcompleted = 1;
				$scope.hideNextcompleted = 0;
			}else if($scope.completedCurrentPage == $scope.completedPages){
			...
			//console.log('----------Completed',$scope.totalItemsCompleted);
		});
		...
		socketFactory.on('corporateScheduledRides', function(data) {
				...
				//console.log('Mode selected trip|newride|returnRide - ',$scope.tripDetailMode,$scope.newRideMode,$scope.returnRideMode);
				...
				if($scope.scheduledTo >= $scope.totalItemsScheduled){
					$scope.scheduledTo = $scope.totalItemsScheduled;
					$scope.hideNextscheduled = 1;
					console.log('Max reached ',$scope.scheduledFrom,' - ',$scope.scheduledTo);
				}
				console.log('Results - ',$scope.scheduledFrom,' - ',$scope.scheduledTo);
				...
				ScheduledRides.forEach(function(ride) {
					...
					if(ride.session_id == 10007 ){console.log(ride.session_id,ride.pickup_time);	}
					...
					if(ride.pickup_time){
						...
						if(ride.session_id == 10007 ){
							console.log(ride.session_id,ride.pickup_time,raw,ride);
						}
						r.tzName = dt.toLocaleString('en', {timeZoneName:'short'}).split(' ').pop();
					}
					...
					if($scope.routeOffID){
						if(r.session_id == $scope.routeOffID){
							console.log('ID FOUND, Routing now');
							$scope.expandDisc(r);
							$scope.showonm(r);	
					...
					if($scope.classExpand == 1){
						if($scope.rideInFocus.session_id == r.session_id){
							//console.log('Tab already expanded', r.session_id);
							$scope.expandDisc(r);
						}
					}
				});	
		...
		socketFactory.on('corporateRegularRides', function(data) {
			...
			if($scope.regularTo >= $scope.totalItemsRegular){
				$scope.regularTo = $scope.totalItemsRegular;
				$scope.hideNext = 1;
				console.log('Max reached ',$scope.regularFrom,' - ',$scope.regularTo);
			}
			console.log('Regular Results - ',$scope.regularFrom,' - ',$scope.regularTo);
			...
			var newRegularData = [];
			//console.log($scope.regular_rides );
			...
			RegularRides.forEach(function(ride) {
				...
				if(ride.ride_status==0 && ride.is_active ==1){
					...
					r.timer = string;
					//console.log(ride.session_id,string);
				}
			...
			//console.log('Rendering Regular Data Now ');
		...
		$scope.monitorLiveData = function (){
				...
				// socketFactory.emit('availableDrivers', {});
				// socketFactory.emit('busyDrivers', {});
				// socketFactory.emit('offlineDrivers', {});
			...
			socketFactory.on('corporateDriverAvailablity', function(data) {
				...
				$scope.availableCarsPages = parseInt($scope.totalAvailableCars);
				...
				if($scope.dsearchFlag && !($scope.routeMode)){
					if($scope.avail[0]){
						...
						map.setCenter(newlatlng);
						$scope.driverZoom = map.getZoom();
						//console.log(map.getZoom(),' but resetting to ',$scope.driverZoom);
						map.setZoom($scope.driverZoom);
					}
				}
		...
		$scope.hideNextAV = 1;
			...
			$scope.driverSearchAPI = function(){
					$.post(MY_CONSTANT.urlC+ 'driver_location', {
						web_access_token:  $cookieStore.get('web_access_token'),				
						driver_name:$scope.searchDriver			
						...
						$rootScope.$apply(function () { 
							...
							if(data.error || data.flag==0){
								...
								$scope.driverToFocus = [];
								$scope.driverToFocus = data;
								if(data.length){
									console.log(data[0].current_location_latitude,data[0].current_location_latitude);
									
									var newlatlng = new google.maps.LatLng(data[0].current_location_latitude, data[0].current_location_longitude);
									...
									$scope.totalAvailableCars = data.length;
			...
			$scope.searchD = function(){
				...
				/*	console.log(markers.length,' clearing');
		...
		/*socketFactory.on('completedRequests', function(data) {
			...
			$scope.sockoc=data[0].data.paginated_rides;
			$scope.completedRides=$scope.sockoc;
			console.log('completedRequests',$scope.completedRides);
		});*/
	...
	$scope.addYourLocationButton = function(map, marker) {
			...
			firstChild.addEventListener('click', function() {
				...
				if(navigator.geolocation) {
					console.log('geoloc',navigator.geolocation);
	...
	$scope.UserShow = false;
		...
		if ($scope.routeState && $scope.routeState.key == routeState.key && $scope.routeState.id == routeState.id) {
			...
			$scope.getDirections();
			console.log('Routed for ',$scope.routeState.id);
		}
	}
	...
	$scope.getDirections = function () {
		...
		directionsService.route(request, function (response, status) {
			...
			if (status === google.maps.DirectionsStatus.OK) {
				directionsDisplay.setDirections(response);
				console.log('Directions displayed for pickup and drop ',$scope.strtloca,$scope.mdes);

				if($scope.fakeThisId == $scope.routeState.id){
					console.log('Yes Need to include Driver.');
					setTimeout(function(){
						$scope.setCenterNow();
					},200);
				}
	...
	$scope.fakeCenter = function(ride,force){			
		console.log('checking if Route Shown ',$scope.routeState.id,ride.session_id);
		if($scope.routeState.id == ride.session_id){
			...
			console.log('Ride Accepted by ',ride.driver_name,' - Routing now',ride.session_id,$scope.strtlocaObj,$scope.mdesObj,$scope.drvObj);
			...
			console.log('Trying to include driver without showing Route');
			$scope.showonm(ride);
			setTimeout(function(){
	...
	$scope.setCenterNow = function() {
		...
		if($scope.drvObj.lat != null){
			...
			console.log('Map Centered Now for Driver and Directions',pointers);
	...
	$scope.cancelRidePopup = function(session_id){
		...
		console.log('cancelling - ',session_id);
	};
	...
	$scope.rescheduleNow = function(){
		...
		console.log($scope.rTrip.pickup_id,$scope.rebooking.time,$scope.rTrip.pickup_latitude,$scope.rTrip.pickup_longitude,$scope.rTrip.manual_destination_latitude,$scope.rTrip.manual_destination_longitude,$scope.rTrip.manual_destination_address);
    ...
    $scope.rescheduleRide = function(ride){
		...
		$scope.rebooking = {};
		console.log($scope.rTrip.pickup_time);
	...
	socketFactory.on('availableDrivers', function(data) {
		...
		if($scope.select.selectedIndex == 3) { //} || $scope.select.selectedIndex == 3) { // for available and all
			...
			for (var i = 0; i < $scope.avl.length; i++) {
				...
				//console.log($scope.avail[i].driver_name,$scope.avail[i].driver_id,image.url)
				if (ongoingDriverIds.includes($scope.avail[i].driver_id)) {
					image.url = `${urlGoing}#${$scope.avail[i].driver_id}`;
				}
				setMarker($scope.avail[i], { image, infowindow, rotation })
	...
	socketFactory.on('offlineDrivers', function(data) {
			...
			for (var i = 0; i < $scope.offline.length; i++) {
				...
				var infowindow = new google.maps.InfoWindow({
					content: "<div style='font-size: 9px;'>" +
					"<span></span>  <span>" + $scope.offline[i].driver_name + "</span><br>" +                            
					"</div>",
					disableAutoPan: true
				});
